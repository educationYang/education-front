...
<c:choose>
	<c:when test="${empty interceptLength}">
		<a href="<product:productUrl product="${product}" category="${category}" />?flag=${flag}" title="<c:out value="${pageScope.productName}"/>">${pageScope.productName}</a>
	</c:when>
	<c:otherwise>
		<a href="<product:productUrl product="${product}" category="${category}" />?flag=${flag}" title="<c:out value="${pageScope.productName}"/>"><app:interceptString length="${interceptLength}" str="${pageScope.productName}"></app:interceptString></a>
	</c:otherwise>
</c:choose>
</c:if>
