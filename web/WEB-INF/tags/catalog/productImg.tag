...
<c:if test="${empty noLink&&empty isUrlOnly}"><a base target="_blank" href="<product:productUrl product="<%=product%>" category="${pageScope.category}"/>?flag=${flag}" <c:if test="${not empty pageScope.alt}"> title="<c:out value="${pageScope.alt}"/>"</c:if>></c:if>
